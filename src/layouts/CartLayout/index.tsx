...
const CartLayout = ({ products }: Props) => {
      ...
      {!isPayment ? (
        <div className="lg:px-[40px] px-[20px] w-full lg:pb-[60px] pb-[20px]">
            <div className="flex-box flex-box--between">
                <div className="w-60">
                  <h3 className="font-PlusJakartaSansSemiBold text-[20px] text-[#000000] uppercase lg:mb-32px">
                    Giỏ hàng
                  </h3>
                  {cartCustomer?.length > 0
                  ? cartCustomer?.map((item: ICart, index : number) => (
                      <ShoppingCard key={`card-item-` + index} item={item} />
                    ))
                  : products
                      ?.slice(0, 3)
                      ?.map((item: ICart, index) => (
                        <ShoppingCard key={`card-item-` + index} item={item} />
                      ))
                  }
                </div>
                <div className="w-35">
                  ...
                  <div className="w-full flex items-center mb-8px">
                    <span className="w-full text-[#616161] text-[14px]">3 Sản phẩm</span>
